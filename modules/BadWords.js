...
module.exports.checkMessage = (Bot, message, options) => {
 ...
 let warnCount = global.Bot.Database.get(`warnings_${message.guild.id}_${user}`)
...
try {
 ...
 if(warnCount === null) {
    global.Bot.Database.set(`warnings_${message.guild.id}_${user}`, 1)
 }
} catch (errr) {
...
if (foundInTextWord){
   if(message.member.hasPermission("ADMINISTRATOR")) return;
      warnCount = global.Bot.Database.add(`warnings_${message.guild.id}_${user}`, 1)
          message.delete();
