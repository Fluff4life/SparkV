...
exports.run = async (bot, message) => {
    ...
    const AntiSwear =
        bot.config.Debug.Enabled === true
            ? false
            : data.guild.settings.automod.removeProfanity;
    ...
    const ChatBot =
        bot.config.Debug.Enabled === true ? null : data.guild.settings.chatbot;
    ...
    if (message.mentions.has(bot.user)) {
        ...
        if (commandfile) {
            ...
            const ChatBot =
                bot.config.Debug.Enabled === true
                    ? true
                    : data.guild.settings.chatbot;

            if (
                ChatBot.toLowerCase() === `mention` &&
...
async function HandleCommand(bot, message, args, command, data, commandfile) {
    ...
    if (commandfile.config.bot_permissions) {
        ...
        if (
            !BotPermisions ||
            !BotPermisions.has(
                Discord.Permissions.FLAGS[commandfile.config.bot_permissions]
            )
        ) {
            return message.reply(
                bot.config.bot.Responses.bot
                    .toString()
                    .replaceAll(
                        `{permission}`,
                        commandfile.config.member_permissions
                    )
            );
        }
    }
    ...
    if (commandfile.config.member_permissions) {
        ...
        if (
            !AuthorPermisions ||
            !AuthorPermisions.has(
                Discord.Permissions.FLAGS[commandfile.config.member_permissions]
            )
        ) {
            return message.reply(
                bot.config.bot.Responses.bot
    ...
    const MusicEnabled =
        bot.config.Debug.Enabled === true
            ? "Enabled"
            : await bot.dashboard.getVal(message.guild.id, `MusicEnabled`);
    const Leveling =
        bot.config.Debug.Enabled === true
            ? "Enabled"
            : await bot.dashboard.getVal(message.guild.id, `leveling`);
    ...
    if (
        ...
        if (UserSlowmode) {
            if (UserSlowmode.time > Date.now()) {
                const delay = message.convertTime(
                    UserSlowmode.time,
                    "to",
                    true
                );
                ...
                UserSlowmode.time =
                    commandfile.config.cooldown * 1000 || 3000 + Date.now();
            }
        } else {
  ...
  */
    ...
    try {
        await commandfile
            .run(bot, message, args, command, data, data)
            .then(async () => {
                const DeleteUsage =
                    bot.config.Debug.Enabled === true
                        ? "Disabled"
                        : await bot.dashboard.getVal(
                              message.guild.id,
                              `deletecommandusage`
                          );
