...
(exports.run = async (bot, message, args, command, data) => {
    ...
    const RocordEnabled = await bot.dashboard.getVal(
        message.guild.id,
        `RocordEnabled`
    );
    ...
    if (RocordEnabled === `Enabled`) {
            ...
            .then((body) => {
                if (body.status === "ok") {
                    const DiscordEmbed = new Discord.MessageEmbed()
                        ...
                        .setDescription(
                            `You've been successfully verified as **${body.robloxUsername}**!`
                        )
                        .setColor(`GREEN`)
        ...
        let PromptEmbed = new Discord.MessageEmbed()
            ...
            .setFooter(
                `This verification prompt will cancel after 200 seconds.`
            )
            .setColor(bot.config.bot.Embed.Color)
            .setTimestamp();
                            ...
                            noblox.getStatus(id).then(async (status) => {
                                noblox.getBlurb(id).then(async (about) => {
                                    if (
                                        ...
                                        const RocordRoleEnabled =
                                            await bot.dashboard.getVal(
                                                message.guild.id,
                                                `RocordVerifyRoleEnabled`
                                            );
                                        ...
                                        if (RocordNicknameTemplate) {
                                            if (
                                                RocordNicknameTemplate.toString().includes(
                                                    `{discord-name}`
                                                )
                                            ) {
                                                RocordNicknameTemplate.toString().replaceAll(
                                                    `{discord-name}`,
                                                    message.author.name
                                                );
                                            }
                                        ...
                                        message.reply(
                                            `Failed to find Verification ID on your status/about page.`
                                        );
                                    }
                                });
                            });
        ...
        return message.reply(
            `Rocord isn't enabled in the dashboard. Please enable it and run this command again!`
        );
    }
}),
    (exports.config = {
