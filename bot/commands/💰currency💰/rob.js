...
exports.run = async (bot, message, args, command, data) => {
  ...
  if (RobberCh1llBucks < 500) {
    return message.reply(`${bot.config.bot.Emojis.error} | Bruh you cannot rob someone unless you have over ❄500 Ch1llBucks.`)
  }
  ...
  if (User.id === process.env.OwnerID) {
    return message.reply(`${bot.config.bot.Emojis.error} | This user is protected! You can buy a protection shield from being robbed in the shop.`)
  }

  if (UserCh1llBucks < 0) {
    return message.reply(`${bot.config.bot.Emojis.error} | This user is in **DEBT**! LOL!! HOW ON EARTH DID THAT HAPPEN LMFAOOOOO!!! Anyways, contact support and we'll reset your balance. :)`)
  }
  ...
  if (Result === `WIN`) {
    ...
    message.reply(`${bot.config.bot.Emojis.success} | You robbed ${User} and recieved ${await bot.FormatNumber(Ammount)} Ch1llBucks!`)
  } else {
