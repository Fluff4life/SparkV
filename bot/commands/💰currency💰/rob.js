...
exports.run = async (bot, message, args, command, data) => {
    ...
    if (!User) {
        return message.reply(
            `${bot.config.bot.Emojis.error} | Please say a person to rob.`
        );
    }
    ...
    if (UserCh1llBucks <= 0 || UserCh1llBucks === null) {
        return message.reply(
            `${bot.config.bot.Emojis.error} | Bruh they have no Ch1llBucks leave them alone you noob!`
        );
    }

    if (message.author.id === User.id) {
        return message.reply(
            `${bot.config.bot.Emojis.error} | Why do you want to rob yourself lol.`
        );
    }
    ...
    if (Result === `WIN`) {
        ...
        message.reply(
            ...
            } | You robbed ${User} and recieved ${await bot.FormatNumber(
                Ammount
            )} Ch1llBucks!`
        );
    } else {
        ...
        message.reply(
            `${bot.config.bot.Emojis.error} | LOL you got caught! You payed ❄250 to ${User}.`
        );
    }
};
exports.config = {
