...
exports.run = async (bot, message, args, command, data) => {
    ...
    if (args[0].toLowerCase() === `all`) {
        if (Ch1llBucks === 0 || Ch1llBucks === null) {
            return message.reply(
                `${bot.config.bot.Emojis.error} | You have no Ch1llBucks!`
            );
        }

        if (Bank === BankMax) {
            return message.reply(
                `${bot.config.bot.Emojis.error} | Your bank is full!`
            );
        }
        ...
        if (Ch1llBucks > BankMax) {
            ...
            message.reply(
                ...
                } | You just deposited ❄${await bot.FormatNumber(
                    BankMax
                )} into your bank!`
            );
        } else {
        ...
        if (!args[0]) {
            return message.reply(
                `${bot.config.bot.Emojis.error} | lol you can't deposit nothing.`
            );
        }
        ...
        if (message.content.includes(`-`)) {
            return message.reply(
                `${bot.config.bot.Emojis.error} | You can't deposit negitive Ch1llBucks lol.`
            );
        }

        if (Ch1llBucks < args[0]) {
            return message.reply(
                `${bot.config.bot.Emojis.error} | You don't have that much Ch1llBucks.`
            );
        }

        if (BankMax < args[0]) {
            return message.reply(
                `${bot.config.bot.Emojis.error} | You don't have enough bank space to hold ❄${args[0]}!`
            );
        }
        ...
        message.reply(
            `${
                bot.config.bot.Emojis.success
            } | Deposited ❄${await bot.FormatNumber(args[0])} into bank!`
        );
    }
};
exports.config = {
