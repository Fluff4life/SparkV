...
exports.run = async (bot, message, args, command, data) => {
  ...
  if (!args) {
    return message.reply(`${bot.config.bot.Emojis.error} | You need to tell me how much you want me to deposit. You can say all if you want all of your Ch1ll Bucks in your bank.`)
  }
  ...
  if (args[0].toLowerCase() === `all`) {
    ...
    if (Ch1llBucks > BankMax) {
      ...
      message.reply(`${bot.config.bot.Emojis.success} | You just deposited ❄${await bot.FormatNumber(BankMax)} into your bank!`)
    } else {
      ...
      message.reply(`${bot.config.bot.Emojis.success} | You just deposited ❄${await bot.FormatNumber(Ch1llBucks)} into your bank!`)
    }
  } else {
    if (!args[0]) {
