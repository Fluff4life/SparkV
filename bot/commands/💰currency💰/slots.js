...
exports.run = async (bot, message, args, command, data) => {
    if (!args) {
        return message.reply(
            `${bot.config.bot.Emojis.error} | lol you need to tell me how much to bet.`
        );
    }
    ...
    if (message.content.includes(`-`)) {
        return message.reply(
            `${bot.config.bot.Emojis.error} | You cannot bet negitive Ch1llBucks lol.`
        );
    }

    if (args[0] > Ch1llBucks) {
        return message.reply(
            `${bot.config.bot.Emojis.error} | You don't have that much lol.`
        );
    }
