...
exports.run = async (bot, message, args, command, data) => {
  const UserToKick = message.guild.member(message.mentions.users.first()) || message.guild.members.cache.get(args[0]) || `@<${args[0]}>`;
  const ReasonForKick = args.join(` `).slice(22) || `No reason provided.`

  if (!args[0]) {
    return message.reply(`${bot.config.bot.Emojis.error} | Please mention someone to kick!`).then(m => m.delete({ timeout: 5000 }))
  }
  ...
  if (UserToKick.id === message.author.id) {
    return message.reply(`${bot.config.bot.Emojis.error} | You cannot kick yourself.`).then(m => m.delete({ timeout: 5000 }))
  }

  if (!UserToKick.kickable) {
    return message.reply(`${bot.config.bot.Emojis.error} | Uh oh... I can't kick this user!`).then(m => m.delete({ timeout: 5000 }))
  }
