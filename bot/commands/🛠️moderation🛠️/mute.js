...
exports.run = async (bot, message, args, command, data) => {
    ...
    if (!MutedRole) {
        if (!message.guild || !message.guild.roles) {
            return message.reply(
                `Uh oh! An error occured. Make sure I have the correct permisions.`
            );
        }
    ...
    if (Emoji === `✅`) {
        ...
        const MuteEmbend = new Discord.MessageEmbed()
            ...
            .setDescription(
                `${bot.config.bot.Emojis.success} | Successfully Muted <@${User.id}>(${User.id})!`
            )
            .setThumbnail(User.avatar)
            .addField(`Moderator/Admin: `, `${message.author.tag}`)
            .addField(`Reason: `, Reason)
            .setFooter(
                `${bot.config.bot.prefix}Unmute to unmute a user. • ${bot.config.bot.Embed.Footer}`
            )
