...
exports.run = async (bot, message, args, command, data) => {
  ...
  if (User.id === message.author.id) {
    return message.reply(`${bot.config.bot.Emojis.error} | You cannot mute yourself.`).then(m => m.delete({ timeout: 5000 }))
  }

  if (!User.kickable) {
    return message.reply(`${bot.config.bot.Emojis.error} | Uh oh... I can't mute this user!`).then(m => m.delete({ timeout: 5000 }))
  }
  ...
  const Emoji = await bot.PromptMessage(VerificationMessage, message.author, [`✅`, `${bot.config.bot.Emojis.error} | `], 60)
