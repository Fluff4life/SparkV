...
exports.run = async (bot, message, args, command, data) => {
  const User = message.guild.member(message.mentions.users.first()) || message.guild.members.cache.get(args[0]) || `@<${args[0]}>`;
  const Reason = args.join(` `).slice(22) || `No reason provided.`

  if (!args[0]) {
    return message.reply(`${bot.config.bot.Emojis.error} | Please mention someone to mute!`).then(m => m.delete({ timeout: 5000 }))
  }
  ...
  if (User.id === message.author.id) {
    return message.reply(`${bot.config.bot.Emojis.error} | You cannot unmute yourself.`).then(m => m.delete({ timeout: 5000 }))
  }

  if (!User.kickable) {
    return message.reply(`${bot.config.bot.Emojis.error} | Uh oh... I can't unmute this user!`).then(m => m.delete({ timeout: 5000 }))
  }

  var Role = message.guild.roles.cache.find(role => role.name.toLowerCase().includes(`muted`))

  if (!Role){
    return message.reply(`${bot.config.bot.Emojis.error} | I couldn't find the muted role! Please make sure the role is called, \`Muted\`.`)
  }
  ...
  if (Emoji === bot.config.bot.Emojis.success) {
    ...
    message.reply(`${bot.config.bot.Emojis.error} | Unmute canceled.`).then(m => m.delete({ timeout: 10000 }))
  }
},
