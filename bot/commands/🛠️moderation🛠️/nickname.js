...
exports.run = async (bot, message, args, command, data) => {
  ...
  if (!args[0]) {
    return message.reply(`${bot.config.bot.Emojis.error} | Please mention someone to change their nickname!`).then(m => m.delete({ timeout: 5000 }))
  }
  ...
  if (!User.roles){
    return message.reply(`${bot.config.bot.Emojis.error} | That\`s not a user! That\`s a role.`).then(m => m.delete({ timeout: 5000 }))
  }

  if (!NewNickname) {
    return message.reply(`${bot.config.bot.Emojis.error} | Please mention their new nickname!`).then(m => m.delete({ timeout: 5000 }))
  }
  ...
  if (Emoji === bot.config.bot.Emojis.success) {
    ...
    User.setNickname(NewNickname).then(() => {
      message.reply(`${bot.config.bot.Emojis.success} | I successfully changed ${User}\`s nickname to ${NewNickname}!`)
    }).catch((err) => {
