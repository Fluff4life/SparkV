...
(exports.run = async (bot, message, args, command, data) => {
    ...
    if (args[0].toLowerCase() === "off" && Queue.filter) {
        bot.distube
            ...
            .then(() =>
                message.reply(
                    `${bot.config.bot.Emojis.error} | Okay, I turned off the filter.`
                )
            );
        ...
        bot.distube
            ...
            .then(() =>
                message.reply(
                    `${bot.config.bot.Emojis.music} | Okay, I turned on filter ${args[0]}.`
                )
            );
    } else {
        return message.reply(
            `${bot.config.bot.Emojis.error} | That's not a valid filter!`
        );
    }
}),
    (exports.config = {
