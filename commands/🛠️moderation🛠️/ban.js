...
exports.run = async (Bot, message, Arguments) => {
  ...
  if (!Arguments[0]) {
    return message.lineReply(`${Bot.Config.Emojis.error} | Please mention someone to ban!`).then(m => m.delete({ timeout: 5000 }))
  }
  ...
  if (UserToBan.id === message.author.id) {
    return message.lineReply(`${Bot.Config.Emojis.error} | You cannot ban yourself.`).then(m => m.delete({ timeout: 5000 }))
  }

  if (!UserToBan.bannable) {
    return message.lineReply(`${Bot.Config.Emojis.error} | Uh oh... I can\`t ban this user!`).then(m => m.delete({ timeout: 5000 }))
  }
  ...
  if (Emoji === Bot.Config.Emojis.error) {
    // Yes
    message.delete()
    UserToBan.send(`${Bot.Config.Emojis.error} | You have been banned from ${message.guild.name}. Reason: ${ReasonForBan}.`).catch((err) => { })

    UserToBan.ban({
      reason: ReasonForBan
    }).catch((err) => {
      message.lineReplyNoMention(`${Bot.Config.Emojis.error} | Failed to ban. Error: ${err}`)
    })
    ...
    const BanEmbed = new MessageEmbed()
      ...
      .setFooter(`${Bot.Config.Bot.prefix}Kick to kick a user. • ${Bot.Config.Embed.EmbedFooter}`)
    ...
    message.lineReplyNoMention(`${Bot.Config.Emojis.error} | Ban canceled.`).then(m => m.delete({ timeout: 10000 }))
  } 
},
