...
exports.run = async (Bot, message, Arguments) => {
  ...
  if (!Channel){
    return message.lineReply("You cannot set slowmode in an announcements channel.")
  }

  if (isNaN(Arguments[0])){
    return message.lineReply("That's not a nunber.").then(m => m.delete({ timeout: 5000 }))
  }

  if (Arguments[0] > 21600){
    return message.lineReply("That's too high of a number! This is due to discord limiting slowmode up to 6 hours.")
  }
