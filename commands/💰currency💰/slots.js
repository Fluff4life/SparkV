...
exports.run = async (Bot, message, Arguments) => {
  if (!Arguments) {
    return message.lineReply(`${Bot.Config.Emojis.error} | lol you need to tell me how much to bet.`)
  }
  ...
  if (message.content.includes(`-`)){
    return message.lineReply(`${Bot.Config.Emojis.error} | You cannot bet negitive Ch1llBucks lol.`)
  }

  if (Arguments[0] > Ch1llBucks){
    return message.lineReply(`${Bot.Config.Emojis.error} | You don't have that much lol.`)
  }
  ...
  if (win){
    message.lineReplyNoMention(`${SlotItems[number[0]]} | ${SlotItems[number[1]]} | ${SlotItems[number[2]]}\n\n${Bot.Config.Emojis.success} | You won ❄${await Bot.FormatNumber(parseInt(Arguments[0]) * 4)} Ch1llBucks!`)
    
    await Bot.Database.add(`UserData.${message.author.id}.ch1llbucks`, Arguments[0] * SlotItems.length)
  } else {
    message.lineReplyNoMention(`${SlotItems[number[0]]} | ${SlotItems[number[1]]} | ${SlotItems[number[2]]}\n\n${Bot.Config.Emojis.error} | You lost ❄${await Bot.FormatNumber(parseInt(Arguments[0]))} Ch1llBucks.`)
