...
async function StartWebsite() {
  ...
  app.use(
    require('serve-favicon')(
      path.resolve(`${MainDir}${path.sep}assets${path.sep}images${path.sep}site${path.sep}favicon.ico`),
    ),
  );
  ...
  app.use((request, response, next) => {
    ...
    Render(response, request, 'error.ejs', {
      head: {
        ...
        SiteDescription:
          "Uh oh! Looks like the page you where looking for wasn't found. Dang man. KingCh1ll is a self-taught coder. He knows html, css, javascript, lua and more!",
        SiteKeywords:
  ...
  app.use((err, request, response, next) => {
    ...
    if (!Config.Debug.Enabled) {
      const MainEmbed = new Discord.MessageEmbed()
        ...
        .addField('**ERROR**', err.toString().replaceAll('', 'USER'), true)
    ...
    Render(response, request, 'error.ejs', {
      ...
      footer: {
        Description:
          'Uh oh! An error occured. KingCh1ll is a self taught developer that enjoys coding. He knows many coding languages.',
      },
    });
  });
