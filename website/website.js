...
async function LoadRoutes() {
    ...
    for (const file of routes) {
        ...
        try {
            ...
            console.error(
                `⛔ | Uh oh! Looks like an error occured with loading route: ${FileName}. \n\n${err}`
            );
        }
    }
}
...
async function StartWebsite() {
  ...
  app.use((request, response, next) => {
    ...
    app.use(
        require("serve-favicon")(
            path.resolve(
                `${process.cwd()}${path.sep}assets${path.sep}images${
                    path.sep
                }site${path.sep}favicon.ico`
            )
        )
    );
    ...
    app.use(
        "/assets",
        express.static(path.join(`${process.cwd()}${path.sep}assets`))
    );
    app.set("views", path.resolve(`${MainDir}${path.sep}views`));
    ...
    app.use((request, response, next) => {
        if (
            !["/login"].includes(request.originalUrl) &&
